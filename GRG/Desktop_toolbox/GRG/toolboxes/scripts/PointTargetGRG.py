...
targetPointOrigin = arcpy.GetParameterAsText(1)
numberCellsHo = arcpy.GetParameterAsText(2)
numberCellsVert = arcpy.GetParameterAsText(3)
cellWidth = arcpy.GetParameterAsText(4)
cellHeight = arcpy.GetParameterAsText(5)
cellUnits = arcpy.GetParameterAsText(6)
gridSize = arcpy.GetParameterAsText(7)
labelStartPos = arcpy.GetParameterAsText(8)
labelStyle = arcpy.GetParameterAsText(9)
...
def main():
    ...
    try:  
        ...
        rightCorner =  float(pointExtents[0]) + ((float(cellWidth) * float(numberCellsHo)) /2.0)
        leftCorner = float(pointExtents[0]) - ((float(cellWidth) * float(numberCellsHo)) /2.0)
        topCorner = float(pointExtents[1]) + ((float(cellHeight) * float(numberCellsVert)) /2.0)
        bottomCorner = float(pointExtents[1]) - ((float(cellHeight) * float(numberCellsVert)) /2.0)
        originCoordinate = str(leftCorner) + " " + str(bottomCorner)
        yAxisCoordinate = str(leftCorner) + " " + str(topCorner)
        ...
        arcpy.CreateFishnet_management(tempOutput, originCoordinate, yAxisCoordinate, 0, 0, str(numberCellsVert), str(numberCellsHo), oppCornerCoordinate, "NO_LABELS", fullExtent, "POLYGON")

define([
    ...
    function (declare, _WidgetsInTemplateMixin, BaseWidget, TabContainer, List, utils, Query, QueryTask, Geoprocessor, FeatureSet, GraphicsLayer, Graphic, Point, SimpleMarkerSymbol, PictureMarkerSymbol, Polyline, SimpleLineSymbol, Polygon, SimpleFillSymbol, Draw, InfoTemplate, esriRequest, graphicsUtils, webMercatorUtils, Color, Dialog, ProgressBar, NumberSpinner, lang, on, dom, domStyle, Select, TextBox, jsonUtils, Chart, Default, Lines, Bars, Pie, Columns, Tooltip, easing, MouseIndicator, Highlight, MoveSlice, MiamiNice, Magnify, array, html, RelationParameters, FeatureLayer, DrawBox, query, domConstruct, FacilitiesPane) {
        return declare([BaseWidget, _WidgetsInTemplateMixin], {
            ...
            queryInfrastructureLayer: function (geom) {
                var query = new Query();
                query.spatialRelationship = Query.SPATIAL_REL_INTERSECTS;
                query.geometry = geom;
                query.outFields = this.config.FacilitiesFields;
                query.returnGeometry = true;

                var data = {};
                data.layerInfo = this.config.chartLayers;
                //data.LayerURL = this.config.FacilitiesLayerURL;
                data.geometry = geom;
                //data.outFields = this.config.FacilitiesFields;
...
//                lbl.innerHTML = "Count of Affected Infrastructure: " + features.length;
            ...
            onERGGPExecuteComplete: function (results) {
                ...
                for (var i = 0; i < features.length; i++) {
                    ...
                    if (ergZone === "Initial Isolation Zone") {
                        ...
                        this.queryInfrastructureLayer(sharedPolygon);
                    }
                }
            ...
            windDirectionQTCompleted: function (results) {
                ...
                if (features.length > 0) {
                    if (this.config.WindDirectionField != null) {
                        var feature = features[0];

                        windTo = feature.attributes[this.config.WindDirectionField] + 180;
                        if (windTo > 360)
                            windTo = windTo - 360;

                        var windDirection = "<b> Wind direction (blowing to:) </b>" + windTo;

                        if (this.config.StationNameField != null)
                            weatherStationName = "<b> Station Name: </b>" + feature.attributes[this.config.StationNameField];

                        if (this.config.DateTimeField != null) {
                            var date = new Date(feature.attributes[this.config.DateTimeField] * 1000);
            ...
            startup: function () {
                ...
                this.facilitiesQueryTask = new QueryTask(this.config.FacilitiesLayerURL);
                this.own(on(this.facilitiesQueryTask, "complete", lang.hitch(this, this.facilitiesQueryTaskCompleted)));

                this.windDirectionQueryTask = new QueryTask(this.config.WindDirectionLayerURL);
                ...
                var ergGPChemicalURL = this.config.ERGChemicalGPServiceURL;
                this.ergGPChemicalService = new Geoprocessor(this.config.ERGChemicalGPServiceURL);
                ...
                //erg placard list
                var ergPlacardURL = this.config.ERGPlacardGPServiceURL;
                this.ergGPPlacardService = new Geoprocessor(this.config.ERGPlacardGPServiceURL);
                ...
                this.findNearestWSService = new Geoprocessor(this.config.FindNearestWSServiceURL);

                //spill size option
                var spillOption = [];
                spillOption[0] = {};
                spillOption[0].label = "Large";
                spillOption[0].value = 0;

                spillOption[1] = {};
                spillOption[1].label = "Small";
                spillOption[1].value = 1;
                ...
                timeOption[0].value = 0;

                timeOption[1] = {};
                timeOption[1].label = "Night";
                timeOption[1].value = 1;

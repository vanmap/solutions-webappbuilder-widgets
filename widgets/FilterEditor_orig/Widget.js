...
define([
  ...
  function (declare, lang, array, html, query, esriBundle, on, _WidgetsInTemplateMixin, BaseWidget, MapManager, LayerInfos, Editor, Popup, TemplatePicker, Button, editUtils, FilterEditor) {
    return declare([BaseWidget, _WidgetsInTemplateMixin], {
      ...
      _converConfiguredLayerInfos: function (layerInfos) {
        ...
        function getLayerObjectFromMapByUrl(map, layerUrl) {
          ...
          for (var i = 0; i < map.graphicsLayerIds.length; i++) {
            ...
            if (layerObject.url && layerObject.url.toLowerCase() === layerUrl.toLowerCase()) {
      ...
      _getTemplatePicker: function (layerInfos) {
        var layerObjects = array.map(layerInfos, function (layerInfo) {
      ...
      _getSettingsParam: function () {
        ...
        if (!this._configEditor.useFilterEdit || this._configEditor.useFilterEdit === false) {
      ...
      _createEditor: function () {
        ...
        if (this._configEditor.useFilterEdit && this._configEditor.useFilterEdit === true) {
      ...
      _update: function () {
        ...
        if (this.config.editor.useFilterEdit && this.config.editor.useFilterEdit === true) {
          if (this.editor.searchTemplatePicker) {
            var cols = Math.floor(width / 60);
            this.editor.searchTemplatePicker.attr('columns', cols);
            this.editor.searchTemplatePicker.update();
          }

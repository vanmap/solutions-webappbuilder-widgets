...
function(declare, lang, array, query, on, domConstruct, domStyle, _WidgetsInTemplateMixin, BaseWidgetSetting,
  ...
  return declare([BaseWidgetSetting, _WidgetsInTemplateMixin], {
    ...
    setConfig:function(config){
      ...
      if(validConfig){
        array.forEach(queries, lang.hitch(this, function(singleConfig, index){
          var addResult = this.queryList.addRow({name: singleConfig.name || '', alias: singleConfig.alias || '' });
          var tr = addResult.tr;
    ...
    getConfig: function () {
      ...
      for(var i = 0; i < trs.length; i++){
        var tr = trs[i];
        var aliasCell = tr.cells[0];
        var aliasNode = query(".editable-div", aliasCell)[0];
        tr.singleConfig.alias = aliasNode.innerHTML;
    ...
    _createSingleQuerySetting:function(tr){
      ...
      this.own(on(this.currentSQS, 'name-change', lang.hitch(this, function(queryName){
        //this.queryList.editRow(tr, {name: queryName, alias: queryName});
        this.queryList.editRow(tr, {alias: queryName});
      })));
    ...
    _onAddNewClicked:function(){
      ...
      this.own(on(sourcePopup, 'ok', lang.hitch(this, function(item){
        ...
        var addResult = this.queryList.addRow({name: queryName, alias: queryName});

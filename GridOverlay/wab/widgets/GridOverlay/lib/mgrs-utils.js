...
define([
  ...
  return {
    ...
    /**
     * The zonesDictionary object contains all 1197 unique keys (one for each non-polar MGRS grid zone)
    ...
    _ZonesDictionary: (function() {
      ...
      // Rather than load it through a single, large json text file, we build it here programmatically,
      // one time, on the client

      var zoneNum, zoneLtr, nonPolarGridZone;
      // Per MGRS definition, these are the valid grid zone letters
      var zoneLetters = ['C','D','E','F','G','H','J','K','L','M','N','P','Q','R','S','T','U','V','W','X'];
      ...
      /**
       * Returns the NonPolarGridZone object for the zone.  NOTE: This is only used during the initial creation of '_ZonesDictionary' object
       * @param  {Number} zoneNum                     The MGRS grid zone number
       * @param  {Number} zoneLtrIndex                The index of the MGRS grid zone letter (w.r.t. the zoneLetters object)
      ...
      var ltrZoneToExtent = function(zoneNum, zoneLtrIndex) {
        ...
        // There are several unique MGRS zones which don't follow the standard convention, they are defined here:
     ...
     * @param  {String} [detail]    Either "more" or "less".  For example, when close to the minSpacing threshhold, which way to lean (i.e. "more" detail or "less" detail).  This effects the way rounding is dones within this function.
    ...
    getInterval: function(minSpacing, map, detail) {
      ...
      // create two points, offset vertically from the screen center, used to determine how many meters the minSpacing screen pixels equates to
      ...
      // since the min/max extent of the MGRS grid is -80/84 respectively, limit the computation to that range
      ...
      pt1Lat = pt1.getLatitude();
      pt1Lon = pt1.getLongitude();
      pt2Lat = pt2.getLatitude();
      pt2Lon = pt2.getLongitude();

      // convert to UTM, and determine approximate meter spacing (width)
      utmZone = mgrs.getZoneNumber(pt1Lat, pt1Lon);
      minSpacingWidth = Math.abs(mgrs.LLtoUTM(pt1Lat, pt1Lon, utmZone)[1] - mgrs.LLtoUTM(pt2Lat, pt2Lon, utmZone)[1]);

      // its easier for me to understand what minSpacingLog10 represents by thinking of it in terms of:
      ...
      if (minSpacingWidth > interval * 2) {
        // sometimes, the computed interval is just too small using the above method and needs to be adjusted
    ...
    zonesFromExtent: function(grid) {
      var xmin, ymin, xmax, ymax, minLtr, maxLtr, minNum, maxNum, ltr, idx, rings, polygon, polygon_i, polyline, polyline_i, minXOffset, maxXOffset;
      var mapExtent = grid.map.extent;
      var deg360 = mapExtent.spatialReference.isWebMercator() ? constants.WEBMERCATOR_360 : constants.GEOGRAPHIC_360;
      var labelParameters = {
      ...
      // check to see if the map extent spans the entire globe (longitudinally) and hide the grid if it does
      ...
      // when the map is in wraparound mode, handle each span separately (divided by Int'l Date Line)
      for (var x_offset = minXOffset; x_offset <= maxXOffset; x_offset++) {
        ...
        // find the min and max MGRS grid zone letters by dividing the vertical extent by zone height (normally 8°),
        ...
        for (var num = minNum; num <= maxNum; num++) {
          ...
          for (var l = minLtr; l <= maxLtr; l++) {
            ...
            polygon_i = geometryEngine.intersect(gridGeomUtils.toWebMercator(polygon), mapExtentPolygon);
            if (polygon_i) {
              // get the border of the visible zone (can be null)
              polyline_i = geometryEngine.intersect(gridGeomUtils.toWebMercator(polyline), mapExtentPolygon);
    ...
    processZonePolygons: function(visibleGridZones, grid) {
      ...
      for (i = 0; i < visibleGridZones.length; i++) {
        visibleGridZone = visibleGridZones[i];
        if (visibleGridZone) {
          // This is the handler for scenario #1 described above
          this.handleZonePolygon(visibleGridZone, grid);

          // if intervalSpacing == 0 (or false), then processing can stop as no 100K grids or intervals will be drawn
          if (intervalSpacing) {
            if (!utmZonePolygons[visibleGridZone.nonPolarGridZone.utmZone]) {
              ...
              // from scenario #2 above, the UTM zone instance already exists, and will be merged with the new data

              // the polygon property is the union of the visibleGridZone geometry and the already existing geometry
              utmZonePolygons[visibleGridZone.nonPolarGridZone.utmZone].polygon = geometryEngine.union([
      ...
      // after processing each visibleGridZone individually (scenario #1), the following with handle each UTM zone
      // as a single group (scenario #2), only if intervalSpacing is greater than 0 and less than or equal to 100K
      if (intervalSpacing) {
        utmZoneKeys = Object.keys(utmZonePolygons);
        for (var i = 0; i < utmZoneKeys.length; i++) {
          var utmZone = utmZoneKeys[i];
          ...
          // using the UTM coordinates, find the min/max values (index 0 of a UTM point is easting, 1 is northing)
          var minEasting = Math.min(lowerLeftUtm[0], lowerRightUtm[0], upperRightUtm[0], upperLeftUtm[0]);
          var maxEasting = Math.max(lowerLeftUtm[0], lowerRightUtm[0], upperRightUtm[0], upperLeftUtm[0]);
          var minNorthing = Math.min(lowerLeftUtm[1], lowerRightUtm[1], upperRightUtm[1], upperLeftUtm[1]); // - 10000;
          var maxNorthing = Math.max(lowerLeftUtm[1], lowerRightUtm[1], upperRightUtm[1], upperLeftUtm[1]);
    ...
    /**
     * An object that contains arguments used during 100K and interval handler methods, allowing objects to be passed by reference
     * @typedef {Object} module:mgrs-utils~MgrsGridHandlerArguments
     * @property {external:Polygon} polygon   A geometry Polygon object that represents the visible grid zone
     ...
     * @param  {module:mgrs-utils~MgrsGridHandlerArguments} args An object holding the arguments for the various handlers
     *
     * @todo Implement configurable parameter for when to label (center and corners), based on zoom
     */
    handle100kGrids: function(args) {
      ...
      for (n = Math.floor(minN / 100000) * 100000; n < maxN; n += 100000) {
        ...
        for (e = Math.floor(minE / 100000) * 100000; e < maxE; e += 100000) {
          // For each 100k increment of n & e, build a 100k by 100k grid polygon, used for labeling and border graphics

          // find the label of the 100K grid
          text = mgrs.findGridLetters(utmZone, (n + 50000 < 0 ? 10000000 + (n + 50000) : n + 50000), e + 50000);

          // Build the 100k grid boundary
          ring = [];
          // start at the bottom left corner, and work east to the bottom right corner (in 10k m increments)
          ...
          // continue adding points to the polygon, working from the bottom right to top right corner
          ...
          // continue adding points to the polygon, working from the top left to the bottom left corner (to close the polygon)
          ...
          clippedPolygon = geometryEngine.intersect(gridGeomUtils.toWebMercator(polygon), gridGeomUtils.toWebMercator(zonePolygon));
          polyline = geometryEngine.intersect(gridGeomUtils.toWebMercator(polyline), gridGeomUtils.toWebMercator(zonePolygon));

          // after being clipped above, they may no longer exist (i.e. they were not within the bounds of the zone)
         ...
         // depending on the map zoom, labels may not be drawn
          if (grid.map.getZoom() < 7) {
            // TODO: 7 should be turned into a configurable number
            continue;
          }

          // depending on the map zoom, only center labels may be drawn
          if (map.getZoom() < 9) {
            // TODO: 9 should be turned into a configurable number
          ...
          if (polyline) {
            ...
            grid._lineGraphics1.push(new Graphic(polyline, new SimpleLineSymbol(SimpleLineSymbol.STYLE_SOLID, new Color(color), lineWidth)));
          }
    ...
    /**
     * Creates graphics and labels for grid interval lines (i.e. lines at less than 100K meter spacing)
     * @param  {module:mgrs-utils~MgrsGridHandlerArguments} args An object holding the arguments for the various handlers
     */
    handleIntervals: function(args) {
      // This method is similar in nature tot he 'handle100kGrids' method, but in the case of intervals,
      // there are no polygons to create, only horizontal and vertical lines.  Thus, much of this code is similar
      // to the 'handle100kGrids' method, without the need for a GridPolygon class object (and less complex labeling logic)
      ...
      var n, e, i, symbologyLevel, lineGraphics, color, lineWidth, fontSize, font, leftPt, rightPt, bottomPt, topPt, path, pt, horizontal, horizontalGraphic, vertical, verticalGraphic, text, labelSymbol, label, screenDistanceBetweenPoints, distanceFromMapEdge;
      ...
      for (n = Math.floor(minN / interval) * interval; n <= maxN; n += interval) {
        ...
        if (n % 100000 === 0) {
          // skip if current increment is 100k (these were already dealt with in the 'handle100kGrids' method)
          continue;
        } else if (n % 10000 === 0) {
          // current increment is 10,000 meters
          symbologyLevel = 2;
          // the graphics array used changes based on symbology level (used to control drawing order)
        ...
        // once the increment has been determined, get the appropriate graphic properties from the grid settings
        color = grid.getColor(symbologyLevel);
        lineWidth = grid.getLineWidth(symbologyLevel);
        fontSize = grid.getFontSize(symbologyLevel);
        font = new Font(fontSize, Font.STYLE_NORMAL, Font.VARIANT_NORMAL, Font.WEIGHT_BOLD, fontFamily);
        ...
        // Create the polyline, from the path created above, and intersect it with the zone polygon.
        // This will result in the visible portion of the line, within the bounds of the zone polygon.
        horizontal = geometryEngine.intersect(
          gridGeomUtils.toWebMercator(new Polyline([path])),
          gridGeomUtils.toWebMercator(zonePolygon));

        // after being clipped above, the polyine may no longer exist (i.e. it was not within the bounds of the zone)
        ...
        if (horizontal) {
          ...
          // add it to the appropriate graphics array (separate arrays are used to control drawing order)
          lineGraphics.push(horizontalGraphic);

          // Build the label(s)

          // find the label text of the interval line
          text = gridLabelUtils.padZero((minN < 0 ? (10000000 + n) : n) % 100000 / interval, 5 - Math.log10(interval));
          ...
          rightPt = new Point({
            "x": path[path.length - 1][0] - 0.001, // a small number is subtracted in order to ensure it is drawn
            "y": path[path.length - 1][1],
            "spatialReference": horizontal.spatialReference
          });

          // determine how far apart the left and right points are from each other (in pixels)
          screenDistanceBetweenPoints = gridLabelUtils.screenDistanceBetweenPoints(leftPt, rightPt, map).horizontal;
          ...
          // if the point is on the edge of the map, or far enough away from the right point, then add label
          if (distanceFromMapEdge < 2 || (distanceFromMapEdge > 200 && screenDistanceBetweenPoints > 300)) {
            labelSymbol = new TextSymbol(text)
            .setAngle(0)
            .setColor(color)
            .setOffset(xOffset, -(fontSize / 1.333) / 2) // -(fontSize/1.333)/2) is the vertical middle
            .setAlign("start")
            .setFont(font);
          ...
          // if the point is on the edge of the map, or far enough away from the left point, then add label
          if (distanceFromMapEdge < 2 || (distanceFromMapEdge > 200 && screenDistanceBetweenPoints > 300)) {
            labelSymbol = new TextSymbol(text)
            .setAngle(0)
            .setColor(color)
      ...
      for (e = Math.floor(minE / interval) * interval; e <= maxE; e += interval) {
        ...
        if (vertical) {
          ...
          screenDistanceBetweenPoints = gridLabelUtils.screenDistanceBetweenPoints(bottomPt, topPt, map).vertical;
          ...
          // if the point is on the edge of the map, or far enough away from the top point, then add label
          if (distanceFromMapEdge < 2 || (distanceFromMapEdge > 200 && screenDistanceBetweenPoints > 300)) {
            labelSymbol = new TextSymbol(text)
          ...
          // if the point is on the edge of the map, or far enough away from the bottom point, then add label
          if (distanceFromMapEdge < 2 || (distanceFromMapEdge > 200 && screenDistanceBetweenPoints > 300)) {
            labelSymbol = new TextSymbol(text)
            .setAngle(verticalLabels ? 90 : 0)
            .setColor(color)

...
define([
    ...
    var c = {
        ...
        ConditionCoord: function (LatORLong) {
        ...
        getGARSletter: function (Ordinal) {
        ...
        LLtoGARS: function (LatitudeDecimal, LongitudeDecimal) {
            ...
            NorthHemisphere = 1;
            if (NorthHemisphere === 1) {
                ConditionedLat = this.ConditionCoord(LatitudeDecimal + 90);
            }

            ConditionedLong = this.ConditionCoord(LongitudeDecimal);

            var LongitudeDegrees = (Math.floor(LongitudeDecimal));
            var LongitudeMinutes = ((LongitudeDecimal - LongitudeDegrees) * 60);

            if (LongitudeMinutes === 0) {
                // Find Longitude Cell
                GARSy = ((180 - LongitudeDegrees) * 2) + 1;
            } else if (LongitudeMinutes <= 30) {
                // Find Longitude Cell
                GARSy = ((180 - LongitudeDegrees) * 2);
            } else {
                // Find Longitude Cell
                GARSy = ((180 - LongitudeDegrees) * 2) - 1;
            }
            ...
            GARSxA = this.getGARSletter(GARSx1);             // Change ordinal number into GARS Cell Letter
            GARSxB = this.getGARSletter(GARSx2);             // Change ordinal number into GARS Cell Letter

            // Determine 1-2-3-4 Quadrant

            var LatitudeDegrees = (Math.floor(LatitudeDecimal));
            var LatitudeMinutes = ((LatitudeDecimal - LatitudeDegrees) * 60);
            ...
            if (GARSlatremainder < 30) {
                if (KeypadRemainder < 30) {
                    Keypad = 1;
                }
                if (KeypadRemainder < 25) {
                    Keypad = 2;
                }
                if (KeypadRemainder < 20) {
                    Keypad = 3;
                }
                if (KeypadRemainder < 15) {
                    Keypad = 1;
                }
                if (KeypadRemainder < 10) {
                    Keypad = 2;
                }
                if (KeypadRemainder < 5) {
                    Keypad = 3;
                }
                if (KeypadRemainder === 0) {
                    Keypad = 1;
                }
            }

...
define([
    ...
    return dojoDeclare(null, {
        ...
        getCoordinateType: function (fromInput) {
            ...
            var strs = [{
                    name: 'DD',
                    pattern: /([-+]?\d{1,3}[.]?\d*[\s]*[°°º^~*]?[\s]*[NnSs]?[\s]*[,]?[\s]*[-+]?\d{1,3}[.]?\d*[\s]*[EeWw]?)/
                }, {
                    name: 'DDM',
                    pattern: /\s?\d{1,3}[°°º^~*]?\s[-+]?\d*\.?\d*\'?[NnSs]?[\s]*[,]?[\s]*\d{1,3}[°°º^~*]?\s[-+]?\d*\.?\d*\'[EeWw]?/
                }, {
                    name: 'DMS',
                    pattern: /([+-]?\d{1,3}[°°º^~*]?[\s,]\d*[']?[\s,]\d*[.]?\d*['"]?[NnSsEeWw]?){1,2}/
                }, {
                //    name: 'GARS',
            ...
            var matchedtype = dojoArray.filter(strs, function (itm) {
                //console.log(itm.name + " : " + this.v.search(itm.pattern));
                return this.v.search(itm.pattern)>=0;

  ...
  function(declare, lang, query, html, array, domClass, domConstruct, on, _WidgetsInTemplateMixin,
    ...
    return declare([BaseWidget, _WidgetsInTemplateMixin], {
      ...
      */
      ...
      //creates a list of layers to use later to match against config.name so that this widget can get the layerObject to filter. Also sets a default def on each layer so that
      //when the widget is reset, the map filter resets.
      ...
      _initSelf:function(){
        ...
        var filters = this.config.queries;
      ...
      // change this function to 1) add function loop through layerset and assign the filter from config
      // 1) check to see if user input is need, otherwise, just execute it.
      _onQueryListClicked:function(event){
        ...
        var filterInfo = singleConfig.filter;
        var filterUtils = new FilterUtils();
        if(filterUtils.isAskForValues(filterInfo)) {
          inputFlag = true;
        }

        this._checkAllLayers({filterObj: singleConfig});

        if(inputFlag){
          this.resetLayerDef();
          this._checkUserInput(singleConfig, tr);
        }
        else{
      ...
      // this is the function to apply the filter, case for Feature Layer and Map Service layers
      ...
      // This function and the next function loops through the layers and map service sublayers and defined the filter based off config
      // For Map services, the filter is applied to the service level, but need the sublayers for other comparisons, hence recursion.
      ...
      // This reverts back the filter on the map with the default and resets the UI.
      ...
      // This function creates the inputs if the filter ask for it. uses filterParameters to do all the
      // type checking.
      _checkUserInput: function(params, tr) {
        ...
        array.forEach(this.filterList, lang.hitch(this, function(lyr) {
          if(lyr.originObject.title === params.name) {
             array.forEach(params.filter.parts, lang.hitch(this, function(part) {
              ...
              if(part.interactiveObj !== "") {
                ...
                var partsObj = lang.clone(params.filter);
                if(lyr.originObject.parentLayerInfo !== null) {
                  var newFL = new FeatureLayer(params.url);
      ...
      // This builds the input value in the definition.  notice for Map services, it has to be added to an array.
